...
INSERT INTO `t_admin_user` (`intID`,`VCH_USER_ID`,`VCH_PASSWORD`,`VCH_FULLNAME`,`VCH_DESIGNATION`,`VCH_ADDRESS`,`VCH_EMAIL`,`VCH_MOBILE`,`DTM_CREATED_ON`,`VCH_CREATED_BY`,`DTM_UPDATED_ON`,`VCH_UPDATED_BY`,`INT_DELETED_FLAG`) VALUES 
 (1,'admin','password','shop admin','Shop Owner',NULL,NULL,NULL,NULL,NULL,NULL,NULL,0x00);
...
/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_admin_login`(in userid varchar(50), in pass varchar(100))
BEGIN
	select vch_user_id,vch_fullname,vch_designation from cmscom.t_admin_user where vch_user_id=userid and vch_password=pass and INT_DELETED_FLAG=0;
END $$
/*!50003 SET SESSION SQL_MODE=@TEMP_SQL_MODE */  $$
